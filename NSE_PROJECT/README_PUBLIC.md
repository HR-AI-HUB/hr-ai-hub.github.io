# NSE Research Drive Workflow met dataminimalisatie

> Notebook-gebaseerde workflow voor het **inlezen, preprocessen, normaliseren, minimaliseren en bevragen** van NSE-data, met **Research Drive (`RD`) als bron- en doellocatie** en een externe AI-inferentieservice voor modelinteractie.

Platform: Windows + VS Code + Jupyter Notebook  
Projectmap: `<project-root>`  
Kernservices: `rclone`, `RD:`, `<ai-hub-base-url>`, `<model-id>`
...
| **Interrogatie / vraag-antwoord** | Leest de geminimaliseerde dataset vanaf Research Drive, bouwt een compacte datasetcontext op en bevraagt via een AI-inferentieservice een geconfigureerd model |
...
NSE_INTERROGATOR_V01.ipynb
        ↓
AI-chat-analyse op geminimaliseerde data
```
...
| Een bruikbaar conda-environment | Zorgt voor reproduceerbare Python-afhankelijkheden |
| Een geldig `.nse-env` bestand in de projectmap | Bevat secrets en padconfiguratie |
| Toegestane toegang tot een AI-inferentieservice | Nodig voor modelcalls in de interrogator |
...
| 3 | `NSE_INTERROGATOR_V01.ipynb` | Geminimaliseerde data analyseren met een AI-modelservice |

### 9. Exacte uitvoerlogica per notebook

#### A. Preprocessing + normalisatie

Open in VS Code de notebook `NSE-AI-HUB_EXCEL_DATA_PREPROSESSING+NORMALIZATION_V1.ipynb`, kies de juiste kernel en voer alle cellen van boven naar beneden uit.
...
```powershell
rclone lsf "RD:<output-root>/"
```

#### B. Minimalisatie

Open `NSE-AIHUB-MINIMALIZER_V01.ipynb` en controleer eerst in de eerste codecel of de invoerbestandsnaam nog naar de juiste preprocessing-output verwijst.

Voorbeeld:

```python
INPUT_FILENAME = "<normalized-output>.csv"
```
...
```powershell
rclone lsf "RD:<output-root>/MINIMIZED"
```
...
```python
INPUT_FILENAME = "nse_minimized_<timestamp>.csv"
```
...
Pas alleen deze variabele aan voor nieuwe analyses, en voer daarna de laatste vraag/antwoord-cellen opnieuw uit.
...
De notebooks verwachten configuratie in `<project-root>/.nse-env`.

Vereiste sleutelset:

```env
WILLMA_API_KEY=<api-key>
WILLMA_BASE_URL=<ai-hub-base-url>
SHARE_LINK=<research-drive-share-link>
SHARE_PASSWORD=<share-password>
RD_OUTPUT_PATH=RD:<output-root>/
```
...
| Variabele | Betekenis |
| --- | --- |
| `WILLMA_API_KEY` | Authenticatie voor de AI-inferentieservice |
...
### Doel van de minimalizer-notebook
...
### Wat de minimalizer-notebook functioneel doet
...
### Doel van de interrogator-notebook
...
### Wat de interrogator-notebook functioneel doet
...
Deze notebook bevraagt de **geminimaliseerde** NSE-dataset met behulp van een AI-inferentieservice en een geconfigureerd model.
...
| Variabele | Doel |
| --- | --- |
| `WILLMA_API_KEY` | Beschikbare authenticatie voor de AI-inferentieservice |
...
Roept de modelcatalogus-endpoint aan om modelbeschikbaarheid te controleren.
...
| `ask_minimized_data(...)` | Vraag en context naar de AI-inferentieservice versturen |

Daarmee wordt een chat payload opgebouwd voor de chat-completions-endpoint.
...
| Interrogator leest dataset succesvol in | Bevestigd |
| Modelcatalogus-endpoint | Werkte |
| Geconfigureerd model beschikbaar | Bevestigd |
| Testvraag naar de AI-inferentieservice succesvol uitgevoerd | Bevestigd |
...
| Beschikbaarheid van het geconfigureerde model via de modelcatalogus | Model moet aanwezig zijn |
...
**Voor intern onderzoeks-, onderwijs- en ontwikkelgebruik binnen de betrokken projectcontext.**
...
Ontwikkelcontext: onderzoeks- en ontwikkelpilot  
Doel: reproduceerbare, privacybewuste verwerking van NSE-data met Research Drive en een AI-inferentieservice
