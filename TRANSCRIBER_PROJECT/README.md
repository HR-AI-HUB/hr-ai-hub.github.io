...
> **Notebook:** `LANGFLOW_WILLMA_WHISPER_TRANSCRIBER_V06.ipynb`  
...
This Langflow flow accepts a **public audio file URL** (pasted into the Langflow Chat Playground), downloads the file into memory, cleans it with a **pure-Python DSP chain**, and sends it to the **SURF WILLMA Whisper API** for transcription and **two-speaker diarization**.
...
| 1 | **WILLMA Audio Downloader** | `requests` (stdlib) | HTTP GET → in-memory `bytes`; handles GitHub raw rewrites |
| 2 | **Audio Preprocessor** | `wave`, `struct`, `math` (stdlib only) | Stereo→mono, resample to 16 kHz, high-pass filter, DRC, peak-normalize |
...
This section walks through building the V06 flow manually. All Python code is sourced from `LANGFLOW_WILLMA_WHISPER_TRANSCRIBER_V06.ipynb`.
...
| Format | Preprocessor DSP | Passed to Whisper |
|--------|-----------------|-------------------|
| 16-bit PCM WAV | ✅ Full pipeline | ✅ |
| WAV with other bit-depth | ⏩ Passthrough (no DSP) | ✅ |
| MP3, M4A, FLAC, OGG | ⏩ Passthrough (no DSP) | ✅ (Whisper handles natively) |

The pure-Python DSP chain only processes **16-bit PCM WAV** files. All other formats are forwarded unchanged — Whisper large-v3 handles them natively server-side.
