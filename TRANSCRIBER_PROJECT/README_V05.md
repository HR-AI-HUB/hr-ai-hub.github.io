...
11. [Supported media formats and URL types](#supported-media-formats-and-url-types)
12. [Rclone access for Research Drive shares](#rclone-access-for-research-drive-shares)
13. [Jupyter notebook implementation](#jupyter-notebook-implementation)
...
The Langflow flow described here accepts a **private (SURF Research Drive) or public audio or video file via an encrypted HTTPS URL** pasted into the Langflow Chat Playground.

It downloads the audio file into memory or, alternatively, extracts audio from video containers via `ffmpeg`, cleans it with a **pure-Python DSP chain**, and sends it to the **SURF WILLMA Whisper API** for transcription and **two-speaker diarization**.
...
#### Graphical interface overview of the **"TRANSCRIBER" flow in the Langflow GUI**
...
| 1 | **WILLMA Audio Downloader** | `requests` | HTTP GET → in-memory `bytes`; handles GitHub raw rewrites, Nextcloud/ownCloud share links, and password-protected SURF Research Drive links via cookie-based session auth *(V07+)* |
...
| Audio / video files accessible by encrypted HTTPS URL | Public links such as GitHub raw file URLs, as well as private, password-protected share links from SURF Research Drive |


#### How to create a password-protected private shared link with Research Drive / Nextcloud
...
[See also SERVICESDESK SURF on REASERCH DRIVE (RD): How to share with other Research Drive user](https://servicedesk.surf.nl/wiki/spaces/WIKI/pages/126222604/RD+How+to+share+with+other+Research+Drive+users)


| Research Drive <br> Next Cloud| output | to-do |
|-------------|-------|----------|
| Click the + sign | Produces a password | Paste the password into component 1: `WILLMA Audio Downloader` |
| Click the + "copy to clipboard" icon <img width="100" alt="image" src="https://github.com/user-attachments/assets/7d209885-c6c6-4d4a-97d3-aaf7e00b8d3a" /> | Produces URL: `https://hr.data.surf.nl/s/E6WT93EcrJFAifE` | Paste the URL into the Playground user input window |
| Copy \<*FILENAME*\>:<br> Ape&nbsp;and&nbsp;Super-Ape&nbsp;&nbsp;Bij&nbsp;de&nbsp;beesten&nbsp;af&nbsp;(Bert Haanstra,&nbsp;1972)&nbsp;[4K].mp4 | Concatenate with the share link: `https://hr.data.surf.nl/s/E6WT93EcrJFAifE/download?path=/\<FILENAME\>` | Paste it into the Playground user input window |
...
4. Open the imported flow. You will see all five components already connected: three custom components and two built-in chat components.
...
This section provides the complete **V07** component code from `LANGFLOW_WILLMA_WHISPER_TRANSCRIBER_V07.ipynb`. The notebook sections later in this document describe earlier Windows-based notebook workflows (`V05e` and `V05f`) that complement, rather than replace, the Langflow implementation.
...
## Rclone access for Research Drive shares

This section documents the validated `rclone` pattern used in the Windows notebook workflow `WILLMA_WHISPER_TRANSCRIBER_V05f.ipynb` for reading from password-protected public SURF Research Drive shares and synchronizing results to a writable `RD:` destination.

It complements the Langflow workflow described above. In the Langflow flow, the downloader component can access public or password-protected share links directly. In the notebook workflow, `rclone` is used to stage shared files into a local cache before transcription starts.

### When to use `rclone`

Use the `rclone` approach when:

- source files are distributed through a public password-protected Research Drive share
- the notebook must process a complete shared batch locally
- outputs must be synchronized to a normal authenticated `RD:` destination
- reproducible CLI-based transfer steps are required

### Read/write model

The validated `V05f` architecture separates the input and output paths:

- **Input:** public password-protected `SHARE_LINK`
- **Staging:** local cache folder on the notebook host
- **Output:** writable authenticated `RD_OUTPUT_PATH`

Important: the public Research Drive share is treated as **read-only**. A direct write test to the public share returned `403 Forbidden`, so transcript outputs should be synchronized to a standard writable `RD:` path instead.

### Required `.whisper-env` settings
...
### Validated inline `rclone` remote pattern

The validated public-share access pattern is:

```text
:webdav,url='https://hr.data.surf.nl/public.php/dav/files/<share_token>',vendor='nextcloud',user='<share_token>',pass='<obscured_password>':
```

Required steps:

1. Extract the share token from `SHARE_LINK`.
2. Run `rclone obscure` on `SHARE_PASSWORD`.
3. Build the inline WebDAV remote for the public share.
4. Copy the shared files to a local cache folder.
5. Run the transcription workflow on the local cache.
6. Synchronize the outputs to `RD_OUTPUT_PATH`.

### PowerShell CLI example

The example below shows the validated CLI flow on Windows PowerShell. It reads the public password-protected share, stages the files in a local cache, and then synchronizes the outputs to a writable `RD:` destination.

```powershell
# Example values — replace with your real settings
$shareLink = 'https://hr.data.surf.nl/s/<share_token>'
$sharePassword = '<share_password>'
$rdOutputPath = 'RD:HR-DATALAB-HEALTHCARE (Projectfolder)/TRANSCRIBED_WILLMA_OUTPUTS/'
$localCache = '.\research_drive_share_cache\input_files'
$localOutput = '.\transcriber_outputs'

# Extract the share token from SHARE_LINK
$shareToken = ($shareLink.TrimEnd('/') -split '/')[-1]

# Obscure the password for inline rclone use
$obscuredPassword = rclone obscure $sharePassword

# Build the inline WebDAV remote for the public share
$shareRemote = ":webdav,url='https://hr.data.surf.nl/public.php/dav/files/$shareToken',vendor='nextcloud',user='$shareToken',pass='$obscuredPassword':"

# Create local staging folders
New-Item -ItemType Directory -Force -Path $localCache | Out-Null
New-Item -ItemType Directory -Force -Path $localOutput | Out-Null

# Copy shared source files into the local cache
rclone copy $shareRemote $localCache --progress

# Example: synchronize generated outputs to the writable RD: destination
rclone copy $localOutput $rdOutputPath --progress
```

If media filtering is required, add `--include` rules for both lowercase and uppercase extensions to avoid skipping files such as `.mp3` and `.MP3`.

### Transfer flow overview

```mermaid
flowchart LR
    A[SHARE_LINK + SHARE_PASSWORD] --> B[Extract share token]
    B --> C[Build rclone WebDAV source]
    C --> D[Copy source files to local cache]
    D --> E[Local audio discovery and validation]
    E --> F[WILLMA transcription and diarization]
    F --> G[Local output artifacts]
    G --> H[Sync results to RD_OUTPUT_PATH]
```

### Operational notes

- `rclone` must be installed and available on `PATH`.
- `rclone listremotes` should contain `RD:` for the writable output destination.
- The local cache functions as a staging area between the shared source and the transcription pipeline.
- Include rules should account for uppercase and lowercase file extensions when filtering shared media files.

### Typical V05f workflow

1. Load `SHARE_LINK`, `SHARE_PASSWORD`, and `RD_OUTPUT_PATH` from `.whisper-env`.
2. Verify that `rclone` is installed and that `RD:` is available.
3. Copy the shared media files into the local cache.
4. Validate the discovered media files locally.
5. Run transcription and diarization.
6. Review the local outputs.
7. Synchronize the outputs to `RD_OUTPUT_PATH`.

---

## Jupyter notebook implementation

The sections below are **added** to the Langflow documentation above.
They document Windows 11-based Jupyter notebook workflows and do not replace the Langflow implementation.

### Scope of these additional sections
...
The source code can be downloaded from the [JUPYTER_NOTEBOOK map](https://github.com/HR-AI-HUB/hr-ai-hub.github.io/tree/main/TRANSCRIBER_PROJECT/JUPYTER_NOTEBOOKS)

Target readers:

- **Data Stewards** — for reproducibility, governance, path validation, and storage controls
- **Data Scientists** — for operational reruns, validation, debugging, and output verification

### Quick comparison

| Version | Input model | Output model | Main purpose |
|---|---|---|---|
| `V05e` | Local folder on disk | Local output folders on disk | Local batch transcription from a prepared audio folder |
| `V05f` | Public password-protected Research Drive share via `rclone` | Local output folder plus sync to writable `RD:` destination | Shared source distribution with managed Research Drive result delivery |

---

## V05e — Local notebook transcription workflow

`V05e` is the local-folder notebook version. It assumes that the source audio files are already available in a local directory that the notebook can access directly.

### When to use V05e

Use `V05e` when:

- source audio has already been exported to a secure local or mapped directory
- no Research Drive share synchronization is needed
- outputs may remain on local or institutionally managed storage

### V05e required configuration

Typical `.whisper-env` content:
...
Before running `V05e`, verify:

1. the required Python packages are installed
2. `ffmpeg` is available on `PATH`
3. `DEFAULT_INPUT_FOLDER` contains supported media files
4. the workstation has write access to the local output location
5. a valid `WILLMA_API_KEY` is configured

### V05e execution steps

1. Activate the notebook environment.
2. Place the source files in `DEFAULT_INPUT_FOLDER`.
3. Run the setup cell.
4. Run the helper-functions cell.
5. Run the discovery / validation step.
6. Run the batch transcription cell.
7. Review the generated local outputs.

### V05e flow

```mermaid
flowchart LR
    A[Local source folder\nDEFAULT_INPUT_FOLDER] --> B[Notebook setup]
...
## V05f — Research Drive notebook transcription workflow

`V05f` extends `V05e` by separating the input source from the output destination:

- source files are read from a public password-protected Research Drive share
- results are created locally and then copied to a writable `RD:` destination

### When to use V05f

Use `V05f` when:

- source files are centrally distributed through Research Drive
- several analysts need to run the same batch from a common source
- final outputs must be synchronized to a managed project folder in Research Drive

### V05f required configuration

Required `.whisper-env` pattern:
...
In addition to the V05e requirements, verify:

1. `rclone` is installed and available on `PATH`
2. `rclone listremotes` contains `RD:`
3. `SHARE_LINK` points to the intended public share
4. `SHARE_PASSWORD` is valid
5. `RD_OUTPUT_PATH` points to the intended writable destination

### V05f execution steps

1. Activate `transcriber-env`.
2. Run the setup cell.
3. Run the helper-functions cell.
4. Run the diagnostic sync cell.
5. Confirm that the shared files were copied into the local cache.
6. Run the batch transcription cell.
7. Review the local outputs.
8. Confirm synchronization to `RD_OUTPUT_PATH`.
...
flowchart LR
    ...
    H --> I[Local outputs\nTXT / JSON / XLSX / SRT / PDF]
    I --> J[Sync to writable RD_OUTPUT_PATH]
    J --> K[Research Drive result folder]
```

### V05f control points

For **Data Stewards**, verify:

- the share contains the expected source batch only
- the password is stored in `.whisper-env`, not in notebook code
- the `RD_OUTPUT_PATH` destination is correct
- local cache and output handling follows project policy

For **Data Scientists**, verify:

- `rclone` works correctly
- the diagnostic step reports a non-zero file count
- uppercase extensions are not skipped by filtering
- the batch run and output sync finish without errors
...
flowchart LR
    ...
    B --> C[Run setup cell]
    C --> D[Run helper cell]
    D --> E[Run diagnostic sync]
    E --> F{Files found?}
    F -- No --> G[Stop and repair access or filtering]
...
## Reproducibility checklist for notebook runs

For both `V05e` and `V05f`, register at least:
...
| Output target | local path or `RD_OUTPUT_PATH` |
| Source file count | `12` |
| Processed file count | `12` |
| Failed file count | `0` |
| Sync status | `success / failed / not applicable` |
| Notes | `rerun after filter fix`, `share updated`, etc. |

### Generic notebook reproducibility flow

```mermaid
flowchart LR
    A[Prepare environment and secrets] --> B[Validate input source]
    B --> C[Run setup]
    C --> D[Run diagnostic or discovery]
...
## Supported media formats and URL types
